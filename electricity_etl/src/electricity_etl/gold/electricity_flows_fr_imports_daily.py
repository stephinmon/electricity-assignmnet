...
TABLE_NAME = "electricity_flows_fr_imports_daily"
...
def transform(silver: DataFrame, contract: EtlContract) -> DataFrame:
    ...
    return daily.filter(F.col("import_mwh") > 0).select(
        F.col("dt"),
        F.col("reference_date"),
        F.col("dest_zone"),
        F.col("dest_zone_name"),
        F.col("country").alias("dest_country"),
        F.col("region").alias("dest_region"),
        F.col("neighbor_zone").alias("source_zone"),
        F.col("neighbor_zone_name").alias("source_zone_name"),
        F.col("neighbor_country").alias("source_country"),
        F.col("neighbor_region").alias("source_region"),
        F.col("import_mwh"),
        F.col("export_mwh"),
